...
import { appUsersTable, activitiesTable, clientOnboardingItemsTable, clientsTable, db, documentsTable } from "@workspace/db";
import { sendChariotEmail } from "../integrations/resend";
import { renderChariotEmail } from "../integrations/email-template";
import { logger } from "../lib/logger";
import { termsAcceptanceText } from "./terms-of-business";
...
const definitions = [
  ...
  { key: "terms_business", label: "Terms of Business", kind: "document", sortOrder: 5 },
  { key: "portfolio", label: "Property portfolio", kind: "document", sortOrder: 6 },
...
export async function getClientOnboarding(clientId: number) {
  ...
  const normalizedItems = items.map((item) => {
    // Terms of Business count as done once accepted (portal / signed copy / staff), not only when a file is uploaded.
    const termsAccepted = item.key === "terms_business" && !!client?.tobAcceptedAt;
    const complete = item.kind === "document"
      ? documentCategories.has(item.key) || termsAccepted
      : Boolean(item.value?.trim());
    const detail = item.key === "terms_business" && client
      ? (termsAcceptanceText(client) ?? (documentCategories.has(item.key) ? "Signed copy uploaded — mark it accepted" : null))
      : null;
    return {
      item,
