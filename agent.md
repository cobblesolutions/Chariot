...
- `artifacts/api-server/src/services/document-reading/` turns uploaded files into structured data. `index.ts` holds the reader registry (`READERS`) and `readDocument` — reads run only on demand (the Read icon on each Add-page document tile → `POST /documents/{id}/read`); uploads never call the model by themselves, so no credit is spent until staff ask and `readingsForDocuments` (attached as `Document.reading` on `ClientDetail`); `checks.ts` compares readings with each other and the record (`ClientDetail.documentChecks`); `xlsx.ts` turns a spreadsheet's first sheet into CSV text.
...
- Live progress: every AI read reports what it is doing to `services/ai-progress.ts` (in-memory, keyed by token). Model calls stream (`integrations/openrouter-stream.ts`, reasoning off, throughput-sorted provider) and `keyProgress` turns each JSON key the model emits into a message (per-reader `progressLabels`; reasoning models' thoughts are shown as "Thinking: …"). Browsers send an `x-ai-progress` token with the request and poll `GET /ai/progress/:token`; background document reads expose the latest line as `DocumentReading.progress`. UI: `AiProgressButton` / `AiProgressLine` + `useAiProgress` in `src/lib/ai-progress.ts` (per-kind fallback sequence until the server speaks). Use them for any new AI action.
